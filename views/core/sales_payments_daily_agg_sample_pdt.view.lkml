  ...
  FROM (
    SELECT
      ...
      IF
        ...
        (DUE_DATE < DATE(@{sample_target_date}) AND AMOUNT_DUE_REMAINING > 0, AMOUNT_DUE_REMAINING, 0)) AS TOTAL_OVERDUE_REMAINING,
      SUM(
      IF
        --(IS_DOUBTFUL, AMOUNT_DUE_REMAINING, 0)) AS TOTAL_DOUBTFUL_REMAINING,
        (DUE_DATE < DATE(@{sample_target_date}) AND AMOUNT_DUE_REMAINING > 0 AND DATE_DIFF(DATE(@{sample_target_date}), DUE_DATE, DAY) > 90, AMOUNT_DUE_REMAINING, 0)) AS TOTAL_DOUBTFUL_REMAINING,
